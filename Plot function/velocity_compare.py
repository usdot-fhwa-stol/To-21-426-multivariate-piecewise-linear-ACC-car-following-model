...
filtered_data = data[(data['Demand'] == 3000) & (data['Av_Pen'].isin([25, 50, 75]))]
...
for column in ['SPEEDAVG_MAX_AV', 'SPEEDAVG_MIN_AV', 'SPEEDAVG_AVG_AV']:
...
def create_complete_bar_plot_with_error_bars(speed_metric, title, ylabel):
    ...
    for i, av_pen in enumerate(av_pen_values):
        ...
        for av in avindex_values:
            ...
            error = (subset['SPEEDAVG_MAX_AV'] - subset['SPEEDAVG_MIN_AV']) / 2
    ...
    ax.set_ylabel('Speed (mph)', fontsize=16)
...
fig_av_complete, ax_av_complete = create_complete_bar_plot_with_error_bars('SPEEDAVG_AVG_AV', 'Average Speed of Automated Vehicles', 'Speed (mph)')

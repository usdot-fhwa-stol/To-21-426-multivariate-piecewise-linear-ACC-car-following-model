...
double Liner_model(double d, double delta_v, double v_f) {
    const double f_d = 0.050;
    const double f_delta_v = 0.478;
    const double f_v = -0.006;
    const double z_0 = -0.968;

    return f_d * d + f_delta_v * delta_v + f_v * v_f + z_0;
}
    ...
    case DRIVER_COMMAND_MOVE_DRIVER :
        if (vehicle_type == 680)
        {
            ...
            if (current_vehicle != vehicle_to_stop)
            {
                ...
                //car_following_acceleration = Liner_model(d / feet2m, v_delta / feet2m, v / feet2m);//liner
                ...
                if (d / (speed_difference - FrontVehAcc + current_acceleration) <= 1 && speed_difference > 0) {

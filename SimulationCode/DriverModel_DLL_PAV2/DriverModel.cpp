...
double MPL(double d, double vf, double v_delta)
/* Gets the acc based on current v,v-delta, and d in  feet */
{
    double accf;
    if (d < 12.1)
    {
        if (vf<24.3)
        { 
            accf = 0.018 * d + 1.398 * v_delta - 0.249 * vf - 0.155;
        }
        else if (vf<48.7)
        { 
            accf = 0.02 * d + 1.101 * v_delta - 0.284 * vf - 0.342;
        }
        else
        {
            accf = 0.036 * d + 1.004 * v_delta - 0.297 * vf - 0.857;
        }
    }
    else
    {
        if (vf < 24.3)
        {
            accf = 0.11 * d + 0.947 * v_delta - 0.1 * vf - 0.129;
        }
        else if (vf < 48.7)
        {
            accf = 0.095 * d + 0.966 * v_delta - 0.132 * vf - 0.271;
        }
        else
        {
            accf = 0.08 * d + 0.802 * v_delta - 0.205 * vf - 0.355;
        }
    }
    return accf;
}
    ...
    case DRIVER_COMMAND_MOVE_DRIVER :
        if (vehicle_type == 660)
        {
            ...
            if (current_vehicle != vehicle_to_stop)
            {
                ...
                double v_delta = -speed_difference;//speed distance v_delta>0 if front veh is faster
                ...
                if (d / (speed_difference - FrontVehAcc + current_acceleration) <= 4 && speed_difference > 0) {
                    desired_acceleration = -speed_difference + FrontVehAcc - current_acceleration;
                }
            ...
            {
                desired_acceleration = min(desired_acceleration, speedlimit - current_velocity);
            }
        }
